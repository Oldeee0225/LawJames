...
export default function HoldingForm() {
  ...
  const holdingCategories = [
    {
      category: sT.holdingCat1,
      entities: [
        { title: sT.holdingCat1E1Title, description: sT.holdingCat1E1Desc, badge: sT.holdingCat1E1Badge, icon: <Globe className="w-6 h-6" /> },
        { title: sT.holdingCat1E2Title, description: sT.holdingCat1E2Desc, badge: sT.holdingCat1E2Badge, icon: <Building2 className="w-6 h-6" /> }
      ]
    },
    {
      category: sT.holdingCat2,
      entities: [
        { title: sT.holdingCat2E1Title, description: sT.holdingCat2E1Desc, badge: sT.holdingCat2E1Badge, icon: <Briefcase className="w-6 h-6" /> }
      ]
    },
    {
      category: sT.holdingCat3,
      entities: [
        { title: sT.holdingCat3E1Title, description: sT.holdingCat3E1Desc, badge: sT.holdingCat3E1Badge, icon: <Landmark className="w-6 h-6" /> },
        { title: sT.holdingCat3E2Title, description: sT.holdingCat3E2Desc, badge: sT.holdingCat3E2Badge, icon: <Network className="w-6 h-6" /> }
      ]
    },
    {
      category: sT.holdingCat4,
      entities: [
        { title: sT.holdingCat4E1Title, description: sT.holdingCat4E1Desc, badge: sT.holdingCat4E1Badge, icon: <Building2 className="w-6 h-6" /> },
        { title: sT.holdingCat4E2Title, description: sT.holdingCat4E2Desc, badge: sT.holdingCat4E2Badge, icon: <Network className="w-6 h-6" /> }
      ]
    }
  ];
        ...
        <div className="max-w-5xl mx-auto relative z-10">
          ...
          <div>
            ...
            <h1 className="text-4xl md:text-6xl font-bold mb-10 leading-tight">
              {sT.holdingFormPageTitle} <span className="gold-gradient italic">{sT.holdingFormPageTitleHighlight}</span>
            </h1>

            <div className="space-y-6 text-gray-400 font-light text-lg leading-relaxed max-w-4xl">
              <p>{sT.holdingFormPageP1}</p>
              <p>{sT.holdingFormPageP2}</p>
              <p>{sT.holdingFormPageP3}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-6 pb-20 relative z-10 mt-12">
        ...
        <div className="mt-32 p-12 rounded-3xl bg-gradient-to-br from-[#0f172a] to-[#020617] border border-[#d4af37]/20 text-center">
          <h2 className="text-3xl font-bold mb-6 italic">
            {sT.holdingFormCTA} <span className="gold-gradient">{sT.holdingFormCTAHighlight}</span>
          </h2>
          <p className="text-gray-400 mb-10 max-w-2xl mx-auto">{sT.holdingFormCTADesc}</p>
          <Link
            to="/contact"
