...
export default function EntityForm() {
  ...
  const entityCategories = [
    {
      category: sT.entityCat1,
      entities: [
        { title: sT.entityCat1E1Title, description: sT.entityCat1E1Desc, badge: sT.entityCat1E1Badge, icon: <Building2 className="w-6 h-6" /> }
      ]
    },
    {
      category: sT.entityCat2,
      entities: [
        { title: sT.entityCat2E1Title, description: sT.entityCat2E1Desc, badge: sT.entityCat2E1Badge, icon: <Globe className="w-6 h-6" /> },
        { title: sT.entityCat2E2Title, description: sT.entityCat2E2Desc, badge: sT.entityCat2E2Badge, icon: <Globe className="w-6 h-6" /> }
      ]
    },
    {
      category: sT.entityCat3,
      entities: [
        { title: sT.entityCat3E1Title, description: sT.entityCat3E1Desc, badge: sT.entityCat3E1Badge, icon: <ShieldCheck className="w-6 h-6" /> },
        { title: sT.entityCat3E2Title, description: sT.entityCat3E2Desc, badge: sT.entityCat3E2Badge, icon: <Briefcase className="w-6 h-6" /> }
      ]
    },
    {
      category: sT.entityCat4,
      entities: [
        { title: sT.entityCat4E1Title, description: sT.entityCat4E1Desc, badge: sT.entityCat4E1Badge, icon: <Users className="w-6 h-6" /> }
      ]
    }
  ];
        ...
        <div className="max-w-5xl mx-auto relative z-10">
          ...
          <div>
            ...
            <h1 className="text-4xl md:text-6xl font-bold mb-10 leading-tight">
              {sT.entityFormPageTitle} <span className="gold-gradient italic">{sT.entityFormPageTitleHighlight}</span> {sT.entityFormPageTitleEnd}
            </h1>

            <div className="space-y-6 text-gray-400 font-light text-lg leading-relaxed max-w-4xl">
              <p>{sT.entityFormPageP1}</p>
              <p>{sT.entityFormPageP2}</p>
              <p>{sT.entityFormPageP3}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-6 pb-20 relative z-10 mt-12">
        ...
        <div className="mt-32 p-12 rounded-3xl bg-gradient-to-br from-[#0f172a] to-[#020617] border border-[#d4af37]/20 text-center">
          <h2 className="text-3xl font-bold mb-6 italic">
            {sT.entityFormCTA} <span className="gold-gradient">{sT.entityFormCTAHighlight}</span>
          </h2>
          <p className="text-gray-400 mb-10 max-w-2xl mx-auto">{sT.entityFormCTADesc}</p>
          <Link
            to="/contact"
